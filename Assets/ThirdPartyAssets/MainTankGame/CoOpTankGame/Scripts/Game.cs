...
public class Game : MonoBehaviour 
{
	...
	public float tankStartMoveSpeed = 1;				//The move speed the player tanks will get when the game starts.
    ...
    public MyTank Player1Tank;							//Player 1's tank. 
    public MyTank Player2Tank;							//Player 2's tank.
	...
	[Header("Components")]
	public GameGUI ui;										//The UI.cs script, located in the GameManager game object.
	...
	void Start ()
	{
		...
		mapToLoad = MapConfigurationManager.Instance.GetMapFile();
	...
	void LoadMap (string map)
	{
        ...
        WallGlobalRoot wallRoot = FindObjectOfType<WallGlobalRoot>();
        SpawnpointGlobalRoot spawnRoot = FindObjectOfType<SpawnpointGlobalRoot>();
        ...
        for (int x = 0; x < lines.Length; x++){				//Loop through all the tiles.
			if(lines[x] != ""){								//Is the line not blank?
				...
				if(parts[0].Contains("Wall")){				//Is this tile a walll?
                    ...
                    wall.name = "Wall (" + wall.transform.position.x + " ; " + wall.transform.position.y + ")";

                    wall.transform.localScale = new Vector3( 151f/1024f, 151f / 1024f, 0);
                    /*BoxCollider2D wallCollider = wall.GetComponent<BoxCollider2D>();
                    if (wallCollider != null)
                        wallCollider.size *= 1024f / 151f;*/
                }
                    ...
                    spawnPoint.transform.parent = spawnRoot.transform;
                    spawnPoint.name = "SpawnPoint (" + spawnPoint.transform.position.x + " ; " + spawnPoint.transform.position.y + ")";
